...
function loadSfInstrument(name: InstrumentName): Promise<SfPlayer> {
  if (!sfCache.has(name)) {
    ...
    sfCache.set(name, Soundfont.instrument(ctx, name, { soundfont: 'MusyngKite', format: 'mp3' }));
  }
...
function getInstrumentFamily(program: number): InstrumentFamily {
  ...
  if (p < 8) return 'piano';
  if (p < 16) return 'chromatic';
  if (p < 24) return 'organ';
  if (p < 32) return 'guitar';
  if (p < 40) return 'bass';
  if (p < 48) return 'strings';
  if (p < 56) return 'ensemble';
  if (p < 64) return 'brass';
  if (p < 72) return 'reed';
  if (p < 80) return 'pipe';
  if (p < 88) return 'synth-lead';
  if (p < 96) return 'synth-pad';
  return 'other';
}
...
function initOscNodes(ctx: AudioContext) {
  if (oscDryBus) return;
  oscDryBus = ctx.createGain();
  oscDryBus.gain.value = 0.7;
  oscDryBus.connect(ctx.destination);
  oscReverbConv = ctx.createConvolver();
  oscReverbConv.buffer = createReverbBuffer(ctx);
  oscWetBus = ctx.createGain();
  oscWetBus.gain.value = 0.25;
  oscReverbConv.connect(oscWetBus);
  oscWetBus.connect(ctx.destination);
}
  ...
  switch (family) {
    case 'piano':
      oscs = [
        ...
        { type: 'triangle', detune: 0, mul: 0.8 },
        { type: 'sine', detune: 2, mul: 0.5 },
      ];
      attack = 0.006;
      decayRatio = 0.35;
      release = 0.6;
    ...
    case 'organ':
      oscs = [
        { type: 'sine', detune: 0 },
        { type: 'sine', detune: 1200, mul: 0.6 },
        { type: 'sine', detune: 1902, mul: 0.4 },
      ];
      attack = 0.008;
      decayRatio = 1.0;
      release = 0.02;
    ...
    case 'pipe':
      oscs = [
        ...
        { type: 'triangle', detune: 7, mul: 0.5 },
      ];
      attack = 0.02;
      decayRatio = 0.9;
      release = 0.1;
    ...
    case 'synth-pad':
      oscs = [
        ...
        { type: 'sawtooth', detune: 7, mul: 0.7 },
      ];
      attack = 0.4;
      decayRatio = 0.7;
      release = 1.2;
...
class MidiPlayer {
  ...
  private async playSoundfont(
    ...
    const trackInstruments = composition.tracks.map(
      (t, i) => t.instrument ?? fallbackPrograms[i % 8]
    );
...
function generateMidiBase64(composition: CompositionData): string {
  ...
  composition.tracks.forEach((trackData, ti) => {
    ...
    trackData.notes.forEach((note) => {
      ...
      const timeSec =
        note.beat !== undefined ? (note.beat - 1) * secondsPerBeat : (note.startTime ?? 0);
...
function renderMarkdown(md: string): string {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith('### ')) {
      closeTable();
      closeList();
      html.push(`<h4>${inlineMd(line.slice(4))}</h4>`);
...
function renderNotation(composition: CompositionData, container: HTMLElement) {
  ...
  const resolvedTracks = tracks.map((track, ti) => {
    ...
    const notes = track.notes.map((note) => {
      ...
      pitches.forEach((p) => {
        ...
        if (m < minPitch) minPitch = m;
        if (m > maxPitch) maxPitch = m;
      });
  ...
  const effMin = minPitch - pad,
    effRange = pitchRange + pad * 2;
  const nh = 10,
    bw = 60,
    lm = 50,
    tm = 20;
  const svgW = lm + totalBeats * bw + 40,
    svgH = tm + effRange * nh + 20;
  ...
  for (let b = 0; b <= totalBeats; b++) {
    ...
    ln.setAttribute('y1', String(tm));
    ln.setAttribute('x2', String(x));
    ln.setAttribute('y2', String(svgH - 20));
    ...
    if (b % 4 === 0) {
      ...
      t.setAttribute('y', String(svgH - 5));
      t.setAttribute('fill', textColor);
      t.setAttribute('font-size', '9');
      t.setAttribute('text-anchor', 'middle');
      t.textContent = String(Math.floor(b / 4) + 1);
      svg.appendChild(t);
    }
  }
  ...
  for (let p = effMin; p <= effMin + effRange; p++) {
    if (p % 12 === 0) {
      ...
      svg.appendChild(t);
      const ln = document.createElementNS('http://www.w3.org/2000/svg', 'line');
      ln.setAttribute('x1', String(lm));
      ln.setAttribute('y1', String(y));
      ln.setAttribute('x2', String(svgW - 40));
...
btnPlay.addEventListener('click', async () => {
  ...
  app.sendLog({
    level: 'info',
    data: `Play (${player.sfMode ? 'soundfont' : 'oscillator'}): ${titleEl.textContent}`,
    logger: 'midi-preview',
  });
});
  ...
  try {
    await app.sendMessage({
      ...
      content: [
        {
          type: 'text',
          text: `Please continue "${title}" by adding 8 more bars. Keep the same key, tempo (${bpm} BPM), and style. Tracks: ${tracks}.`,
        },
      ],
    });
    app.sendLog({
      level: 'info',
      data: `Continue request sent for: ${title}`,
      logger: 'midi-preview',
    });
  } catch (e) {
    app.sendLog({
      level: 'warning',
      data: `sendMessage failed: ${(e as Error).message}`,
      logger: 'midi-preview',
    });
  }
});
...
async function analyzeChord() {
  ...
  try {
    ...
    app.sendLog({
      level: 'info',
      data: `Chord ${chord} → ${data.noteNames.join(', ')}`,
      logger: 'midi-preview',
    });
  } catch (e) {
